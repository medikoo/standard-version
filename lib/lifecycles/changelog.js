  ...
  return new Promise((resolve, reject) => {
    ...
    changelogStream.on('end', function () {
      ...
      if (args.dryRun) {
        ...
        try {
          const result = spawnSync('node', ['node_modules/.bin/prettier', '--write', basename(args.infile), 'package.json'], {
            cwd: dirname(args.infile)
          })
          if (result.status) console.error(`Prettier failed with:\n\n${result.stdout}\n${result.stderr}`)
        } catch (error) {
          console.info(`Could not prettify: ${error.message}`)
        }
      }
      return resolve()
    })
